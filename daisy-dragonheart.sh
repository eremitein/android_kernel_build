...
# 2019 Mod by eremitein@xda / zerovoid@4pda
...
# Main script
while true; do
	echo -e "\n[1] Build Daisy DragonHeart Kernel"
	...
	if [ "$choice" == "1" ]; then
		...
		git clone https://github.com/eremitein/ak2.git -b dragonheart AnyKernel2
		...
		echo -e ""
		if [ -r out/.version ]; then
		  rm out/.version
		fi
		make O=out $CONFIG $THREAD &>/dev/null
		#make V=1 O=out $THREAD 2>&1 | tee kernel.log
		make V=1 CONFIG_DEBUG_SECTION_MISMATCH=y CONFIG_NO_ERROR_ON_MISMATCH=y O=out $THREAD 2>&1 | tee kernel.log
